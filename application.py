...
def login():
	...
	if request.method == 'GET':
		...
		if request.method == 'POST':
			...
			if user and valid_pw(request.form['username'],request.form['password'],user.pw_hash):
			
				login_session['username'] = request.form['username']
				#posts = session.query(Blog).all()
				#return render_template('private.html', posts = posts,username=login_session['username'])
...
def eliminarItem(id):
	...
	if request.method == 'GET':
		...
		return render_template('delete-post.html', post = post,username = username)
	else:
		if request.method == 'POST':
			id_user = session.query(User).filter_by(username=login_session['username']).one()
			if id_user.id == post.user_id:
				session.delete(post)
				session.commit()
				return redirect(url_for('showMain'))
			else:
...
def agregarPost():
	...
	if request.method == 'GET':
		username = login_session['username']
		return render_template('add-post.html',username = username)
	else:
		if request.method == 'POST':
			#usuario = login_session['username'] ---no hizo falta agragar esto------
			id = session.query(User).filter_by(username=login_session['username']).one()
			post = Blog(
...
def editarPost(id):
	...
	if request.method == 'GET':
		username = login_session['username']
		return render_template('edit-post.html',post = post, username = username)
	else:
		if request.method == 'POST':
			#usuario = login_session['username'] ---no hizo falta agragar esto------
			#id = session.query(User).filter_by(username=login_session['username']).one()
...
def showMain():
	...
	if 'username' in login_session:
		...
		id = session.query(User).filter_by(username=login_session['username']).one()
		print id.id
		return render_template('private.html', posts = posts, id = id.id ,username=username)	
	else:
